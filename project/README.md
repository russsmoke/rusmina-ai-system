...
4. Открыть MLflow UI (localhost:5000), показать сравнение моделей
5. Показать ноутбук 03_shap.ipynb с интерпретацией предсказаний
---

## 8. Ограничения и дальнейшая работа

В текущей версии мониторинг ограничен логами (без Grafana/Prometheus), модель переобучается вручную через ноутбук.
В дальнейшем можно добавить:

- автоматическое переобучение по расписанию (Prefect);
