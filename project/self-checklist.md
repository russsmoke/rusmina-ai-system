...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |         ✅         | `src/service/main`, `src/models/load_artifacts.py` строка 36             |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          |         ✅         | `notebooks/01_eda_and_baseline.ipynb`, `report.md` раздел "Экспериментальный протокол и результаты"     |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        |         ✅         | `notebooks/01_eda_and_baseline.ipynb`, `notebooks/02_models_XGBoost.ipynb`, `notebooks/04_models_MLP.ipynb`, `notebooks/05_CatBoost.ipynb`, `report.md` |
