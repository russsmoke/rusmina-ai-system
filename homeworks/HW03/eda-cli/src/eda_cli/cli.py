...
def report(
    ...
    # Находим колонки с высокой долей пропусков (используем min_missing_share)
    high_missing_cols = []
    if not missing_df.empty:
        high_missing_mask = missing_df["missing_share"] > min_missing_share
        high_missing_cols = missing_df[high_missing_mask].index.tolist()
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if "has_constant_columns" in quality_flags:
            ...
            if quality_flags['has_constant_columns']:
                f.write(f"  - Константные колонки: {', '.join(quality_flags.get('constant_columns', []))}\n")
    ...
    typer.echo(f"- Настройки отчёта: max_hist={max_hist_columns}, top_k={top_k_categories}, missing_threshold={min_missing_share}")
