...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        # Пропускаем колонки с пропусками - они не могут быть полностью константными
        if col.missing > 0:
            continue
        # Если все непустые значения одинаковы (уникальных значений = 1)
        if col.non_null > 0 and col.unique == 1:
            has_constant_columns = True
    flags["has_constant_columns"] = has_constant_columns

    # Проверка на категориальные признаки с высокой кардинальностью с порогом в 50 уникальных значений
    has_high_cardinality_categoricals = False
    for col in summary.columns:
        # Определяем, является ли колонка категориальной (объект или категория)
        is_categorical = col.dtype in ['object', 'category', 'string']
        # Также считаем строки с большим числом уникальных значений категориальными
        if (is_categorical or col.unique > 10) and not col.is_numeric:
            if col.unique > 20:  # Порог высокой кардинальности
